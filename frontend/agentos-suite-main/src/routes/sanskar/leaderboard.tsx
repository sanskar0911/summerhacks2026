...
function LeaderboardPage() {
  const [data, setData] = useState<{ freelancers: any[], clients: any[] } | null>(null);
  ...
  const isCurrentUser = (f: any) => user && (f.email === user.email || f.id === user.id);

  if (loading) return <div className="p-20 text-center font-bold animate-pulse">Synchronizing Global Rankings...</div>;
      ...
      <header className="space-y-4">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground font-medium text-lg">Top-performing freelancers and highest-spending clients this month.</p>
          </div>
          <div className="flex items-center gap-4 bg-white/5 border border-white/5 px-6 py-4 rounded-[1.5rem]">
             <div className="text-right">
                <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Active Talent</div>
                <div className="text-xl font-black text-white">{data?.freelancers.length || 0} Elite</div>
             </div>
             <div className="h-8 w-px bg-white/10" />
             <div>
                <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Global Volume</div>
                <div className="text-xl font-black text-success">₹4.2Cr</div>
             </div>
          </div>
        </div>
      </header>
